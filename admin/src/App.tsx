...
import { getUserRoles } from '../../src/utils/adminAuth';
...
const App: React.FC = () => {
    ...
    const loadUsers = async () => {
        ...
        try {
            ...
            // Debug: Check roles
            try {
                currentRoles = await getUserRoles();
            ...
            setDebugInfo(`加载用户失败: ${error.message}. Roles: [${currentRoles.join(', ')}]`);
        } finally {
    ...
    return (
        <AdminProtectedRoute>
            <LoadingStateProvider>
                <div className="flex h-screen bg-[#f8f8f5] dark:bg-[#111418] overflow-hidden relative">
                    {/* Debug Banner */}
                    {debugInfo && (
                        <div className="absolute top-0 left-0 right-0 bg-red-500 text-white p-2 z-[100] text-center text-sm font-bold shadow-md">
                            {debugInfo}
                            <button onClick={() => setDebugInfo('')} className="ml-4 underline">关闭</button>
                            <button onClick={loadUsers} className="ml-4 border border-white px-2 rounded">重试</button>
                        </div>
                    )}
                    {/* Desktop Sidebar */}
                    <aside
                    ...
                    {/* Mobile Sidebar */}
                    <aside
                        className={`mobile-menu fixed top-0 left-0 h-full w-64 bg-white dark:bg-[#1c2127] z-50 transform transition-transform duration-300 md:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                            }`}
                    >
                            ...
                            <div className="flex items-center justify-around px-2 py-2">
                                {menuItems.slice(0, 5).map((item) => (
                                    <button
                                        ...
                                        onClick={() => handleViewChange(item.id)}
                                        onTouchStart={() => item.id !== activeView && preloadAdminView(item.id)}
                                        className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all min-w-[60px] ${activeView === item.id
                                                ? 'text-primary'
